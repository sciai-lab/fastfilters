	...
	for (unsigned int dim = 0; dim < n_times; ++dim) {
		...
		for (; i < avx_end; i += 32) {
			...
			for (unsigned int j = 1; j <= half_kernel_len; ++j) {
				...
				// sum pixels for both sides of kernel (kernel[-j] * image[i-j] + kernel[j] * image[i+j] = (image[i-j] + image[i+j]) * kernel[j])
				...
				if (is_symmetric) {
					...
					pixels1 = _mm256_add_ps(_mm256_loadu_ps(cur_input + i + j + 8), _mm256_loadu_ps(cur_input + i - j + 8));
					pixels2 = _mm256_add_ps(_mm256_loadu_ps(cur_input + i + j + 16), _mm256_loadu_ps(cur_input + i - j + 16));
					pixels3 = _mm256_add_ps(_mm256_loadu_ps(cur_input + i + j + 24), _mm256_loadu_ps(cur_input + i - j + 24));
				} else {
					pixels0 = _mm256_sub_ps(_mm256_loadu_ps(cur_input + i + j), _mm256_loadu_ps(cur_input + i - j));
					pixels1 = _mm256_sub_ps(_mm256_loadu_ps(cur_input + i + j + 8), _mm256_loadu_ps(cur_input + i - j + 8));
					pixels2 = _mm256_sub_ps(_mm256_loadu_ps(cur_input + i + j + 16), _mm256_loadu_ps(cur_input + i - j + 16));
					pixels3 = _mm256_sub_ps(_mm256_loadu_ps(cur_input + i + j + 24), _mm256_loadu_ps(cur_input + i - j + 24));
				}
...
void convolve_fir_inner_single_avx(
	const float *input,
	const unsigned int n_pixels, const unsigned n_times,
	float *output,
	const float *kernel, const unsigned int kernel_len)
	...
	const float *kernel, const unsigned int kernel_len)
{
	...
	for (dim = 0; dim < dim_avx_end; dim += 8) {
		...
		for (; i < pixels_avx_end; i+= 4) {
			...
			for (unsigned int j = 1; j <= half_kernel_len; ++j) {
				...
				if (is_symmetric) {
					pixels0 = _mm256_add_ps(_mm256_loadu_ps(input + dim + (i+j+0)*n_pixels), _mm256_loadu_ps(input + dim + (i-j+0)*n_pixels));
					pixels1 = _mm256_add_ps(_mm256_loadu_ps(input + dim + (i+j+1)*n_pixels), _mm256_loadu_ps(input + dim + (i-j+1)*n_pixels));
					pixels2 = _mm256_add_ps(_mm256_loadu_ps(input + dim + (i+j+2)*n_pixels), _mm256_loadu_ps(input + dim + (i-j+2)*n_pixels));
					pixels3 = _mm256_add_ps(_mm256_loadu_ps(input + dim + (i+j+3)*n_pixels), _mm256_loadu_ps(input + dim + (i-j+3)*n_pixels));
				} else {
					pixels0 = _mm256_sub_ps(_mm256_loadu_ps(input + dim + (i+j+0)*n_pixels), _mm256_loadu_ps(input + dim + (i+j+0)*n_pixels));
					pixels1 = _mm256_sub_ps(_mm256_loadu_ps(input + dim + (i+j+1)*n_pixels), _mm256_loadu_ps(input + dim + (i-j+1)*n_pixels));
					pixels2 = _mm256_sub_ps(_mm256_loadu_ps(input + dim + (i+j+2)*n_pixels), _mm256_loadu_ps(input + dim + (i-j+2)*n_pixels));
					pixels3 = _mm256_sub_ps(_mm256_loadu_ps(input + dim + (i+j+3)*n_pixels), _mm256_loadu_ps(input + dim + (i-j+3)*n_pixels));
				}
	...
	const  __m256i mask = _mm256_set_epi32(
			0,
			dim_left >= 7 ? 0xffffffff : 0,
			dim_left >= 6 ? 0xffffffff : 0,
			dim_left >= 5 ? 0xffffffff : 0,
			dim_left >= 4 ? 0xffffffff : 0,
			dim_left >= 3 ? 0xffffffff : 0,
			dim_left >= 2 ? 0xffffffff : 0,
			0xffffffff
		);
	...
	for (; i < pixels_avx_end; i+= 4) {
		...
		for (unsigned int j = 1; j <= half_kernel_len; ++j) {
			...
			if (is_symmetric) {
				pixels0 = _mm256_add_ps(_mm256_maskload_ps(input + dim + (i+j+0)*n_pixels, mask), _mm256_maskload_ps(input + dim + (i-j+0)*n_pixels, mask));
				pixels1 = _mm256_add_ps(_mm256_maskload_ps(input + dim + (i+j+1)*n_pixels, mask), _mm256_maskload_ps(input + dim + (i-j+1)*n_pixels, mask));
				pixels2 = _mm256_add_ps(_mm256_maskload_ps(input + dim + (i+j+2)*n_pixels, mask), _mm256_maskload_ps(input + dim + (i-j+2)*n_pixels, mask));
				pixels3 = _mm256_add_ps(_mm256_maskload_ps(input + dim + (i+j+3)*n_pixels, mask), _mm256_maskload_ps(input + dim + (i-j+3)*n_pixels, mask));
			} else {
				pixels0 = _mm256_sub_ps(_mm256_maskload_ps(input + dim + (i+j+0)*n_pixels, mask), _mm256_maskload_ps(input + dim + (i+j+0)*n_pixels, mask));
				pixels1 = _mm256_sub_ps(_mm256_maskload_ps(input + dim + (i+j+1)*n_pixels, mask), _mm256_maskload_ps(input + dim + (i-j+1)*n_pixels, mask));
				pixels2 = _mm256_sub_ps(_mm256_maskload_ps(input + dim + (i+j+2)*n_pixels, mask), _mm256_maskload_ps(input + dim + (i-j+2)*n_pixels, mask));
				pixels3 = _mm256_sub_ps(_mm256_maskload_ps(input + dim + (i+j+3)*n_pixels, mask), _mm256_maskload_ps(input + dim + (i-j+3)*n_pixels, mask));
			}
...
void convolve_fir_outer_single_avx(
	const float *input,
	const unsigned int n_pixels, const unsigned n_times,
	float *output,
	const float *kernel, const unsigned int kernel_len)
