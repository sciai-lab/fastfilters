...
class run_py_script(Task.Task):
	...
	run_str = '${PREPEND} "${PYCMD}" "${SRC[0].abspath()}" ${APPEND} "${TGT[0].abspath()}"'
	shell = True
...
def apply_run_py_script(tg):
	...
	ldpath = []
	if getattr(tg, 'add_to_ldpath', None):
		ldpath.append(tg.add_to_ldpath)


	def add_to_path(env, vname, lst):
		if env.get(vname, None):
			newpath = env[vname].split(os.pathsep)
		else:
			newpath = []
		newpath += lst
		env[vname] = os.pathsep.join(newpath)


	if Utils.is_win32:
		add_to_path(tsk.env.env, 'PATH', ldpath)
	elif Utils.unversioned_sys_platform() == 'darwin':
		add_to_path(tsk.env.env, 'DYLD_LIBRARY_PATH', ldpath)
		add_to_path(tsk.env.env, 'LD_LIBRARY_PATH', ldpath)
	else:
		add_to_path(tsk.env.env, 'LD_LIBRARY_PATH', ldpath)
